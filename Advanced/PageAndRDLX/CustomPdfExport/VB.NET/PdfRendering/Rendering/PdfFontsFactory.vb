...
Friend NotInheritable Class PdfFontsFactory
	...
	Public Function GetPdfFont(font As FontInfo) As XFont
		...
		Catch generatedExceptionName As Exception
			...
			Catch generatedExceptionName2 As Exception
				resultFont = New XFont("Arial", font.Size, GetFontStyle(font), _options)
			End Try
		End Try
	...
	Private Shared Function GetFontStyle(fontInfo As FontInfo) As XFontStyleEx
		Dim resultStyle As XFontStyleEx = XFontStyleEx.Regular
		If (fontInfo.Weight And FontWeight.Bold) <> 0 Then
			resultStyle = resultStyle Or XFontStyleEx.Bold
		End If
		If (fontInfo.Style And Drawing.FontStyle.Italic) <> 0 Then
			resultStyle = resultStyle Or XFontStyleEx.Italic
		End If
		If (fontInfo.Decoration And FontDecoration.Linethrough) <> 0 Then
			resultStyle = resultStyle Or XFontStyleEx.Strikeout
		End If
		If (fontInfo.Decoration And FontDecoration.Underline) <> 0 Then
			resultStyle = resultStyle Or XFontStyleEx.Underline
		End If
		Return resultStyle
	End Function
			...
			Using font As Font = New Font(fontName, 80, fontStyle)
				...
				Using gTemp As Graphics = Graphics.FromImage(_refBitmap)
					...
					Try
						...
						If fontDataSize = 0 OrElse fontDataSize = 4294967295 Then Return Nothing
						Dim fontData = New Byte(CInt(fontDataSize - 1)) {}
						Dim result = GetFontData(hdc, table, 0, fontData, CInt(fontDataSize))
						If result = 4294967295 Then Return Nothing
						Return fontData
					Catch
						Return Nothing
					Finally
