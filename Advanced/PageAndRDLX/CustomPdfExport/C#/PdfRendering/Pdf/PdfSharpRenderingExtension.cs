		...
		void IRenderingExtension.Render(IReport report, StreamProvider streamProvider, NameValueCollection settings, CancellationToken cancel)
		{
			...
			try
			{
				...
				using (var generator = new PdfGenerator(outputStream, pdfSettings.EmbedFonts))
				{
					...
					var generatorSettings = new PageGeneratorSettings()
					{
						TargetDeviceKind = pdfSettings.Target,
						IncludePageMargins = true,
						PageSettings = new GrapeCity.ActiveReports.ImageRenderers.PageControl.PageControllerSettings() { EndPage = endPage, StartPage = startPage },
						IsDelayedContentSupported = true,
						InteractivitySupport = InteractivityType.BuiltinHyperlinks
					};
					
					renderingCore.Render(generatorSettings);
				}
			}
			catch
			{
