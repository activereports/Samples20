...
namespace ActiveReports.Samples.Export.Rendering.PdfSharp
{
	internal sealed class PdfFontsFactory
	{
		...
		private static XFontStyleEx GetFontStyle(FontInfo fontInfo)
		{
			XFontStyleEx resultStyle = XFontStyleEx.Regular;
			if ((fontInfo.Weight & FontWeight.Bold) != 0)
				resultStyle |= XFontStyleEx.Bold;
			if ((fontInfo.Style & FontStyle.Italic) != 0)
				resultStyle |= XFontStyleEx.Italic;
			if ((fontInfo.Decoration & FontDecoration.Linethrough) != 0)
				resultStyle |= XFontStyleEx.Strikeout;
			if ((fontInfo.Decoration & FontDecoration.Underline) != 0)
				resultStyle |= XFontStyleEx.Underline;
			return resultStyle;
		}
