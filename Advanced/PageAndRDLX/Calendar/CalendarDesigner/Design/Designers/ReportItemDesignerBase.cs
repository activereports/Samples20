...
using ActiveReports.Calendar.Rendering;
using GrapeCity.ActiveReports;
using GrapeCity.ActiveReports.Design.DdrDesigner.Designers;
using GrapeCity.ActiveReports.Internal;
...
namespace ActiveReports.Calendar.Design.Designers
{
    /// <summary>
    ...
    public  class ReportItemDesignerBase : CustomReportItemDesigner
	{
		...
		public sealed override void PostFilterProperties(IDictionary properties)
		{
			...
			Apply(properties, PropertyDescriptors, null, GetDesignerHost());
		}
		
		private static void Apply(IDictionary properties,
			IEnumerable<PropertyDescriptor> descriptors,
			Func<string, bool> nameFilter, IDesignerHost host)
		{
			Apply(
				properties,
				from pd in descriptors
				select new KeyValuePair<string, PropertyDescriptor>(pd.Name, pd),
				nameFilter, host
			);
		}
		
		private static void Apply(IDictionary properties,
			IEnumerable<KeyValuePair<string, PropertyDescriptor>> descriptors,
			Func<string,bool> nameFilter, IDesignerHost host)
		{
			PageReport report = null;
			Func<PageReport> resolveReport = () =>
			{
				if (report != null) return report;
				return host != null ? (report = host.RootComponent as PageReport) : null;
			};

			Apply(properties, descriptors, nameFilter, resolveReport);
		}
		
		private static void Apply(IDictionary properties,
			IEnumerable<KeyValuePair<string,PropertyDescriptor>> descriptors,
			Func<string, bool> nameFilter, Func<PageReport> resolveReport)
		{
			if (nameFilter == null)
			{
			...
			{
				if (nameFilter(property.Key) && IsBrowsable(property.Value, resolveReport))
				{
		...
		private static bool IsBrowsable(PropertyDescriptor property, Func<PageReport> reportFn)
		{
			if (IsFplOnly(property))
			{
				return IsFpl(reportFn());
			}
			if (IsCplOnly(property))
			{
				return !IsFpl(reportFn());
			}
			return true;
		...
		public static bool IsFpl(PageReport report)
		{
			return Utils.IsFplReport(report);
		}
		
		private static bool IsFplOnly(PropertyDescriptor pd)
		{
			return pd != null && pd.Attributes.Cast<Attribute>().Any(x => x is FplOnlyAttribute);
		}

		private static bool IsCplOnly(PropertyDescriptor pd)
		{
			return pd != null && pd.Attributes.Cast<Attribute>().Any(x => x is CplOnlyAttribute);
		}
