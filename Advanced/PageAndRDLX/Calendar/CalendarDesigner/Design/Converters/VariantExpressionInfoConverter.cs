    ...
    internal class VariantExpressionInfoConverter : ExpressionInfoConverterBase
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
		{
			if (value is string)
			{
				...
				ExpressionInfo expressionInfo = ExpressionInfo.FromString(value as string, ExpressionResultType.Variant);
				if (context == null)
				...
				if (service != null)
				{
					...
					if (expressionInfo.IsConstant)
					{
						if (expressionInfo.ResultType == ExpressionResultType.String && !IsValueExpression((string)value))
							return ExpressionInfo.FromString((string)value, ExpressionResultType.String);
						return ExpressionInfo.FromString(Convert.ToString(service.Evaluate(expressionInfo), CultureInfo.InvariantCulture), expressionInfo.ResultType);
					}
				}
				return expressionInfo;
			}
